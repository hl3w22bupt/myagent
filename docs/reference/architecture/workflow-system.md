...
WorkflowEngine 在执行每个 step 时，会为整个 workflow 创建一个共享的 workspace：

```
Workflow 开始
    ↓
第一个 step: resolveWorkspace()
    ↓
创建 workspace 目录 (mkdirSync)
    ↓
存储到 context.variables.workflowWorkspace
    ↓
后续 step 共享同一 workspace
```

### Workspace 来源判断

```typescript
// 如果用户在 environment 中指定了 workspace
const userWorkspace = options.environment?.workspace;

// 系统自动生成（用户未指定）
const autoWorkspace = getDefaultWorkspace(taskId);
...
Workflow 完成后，根据 `workspaceAutoGenerated` 标记决定是否清理：

| 情况 | workspaceAutoGenerated | 操作 |
|------|----------------------|------|
| 用户指定 `environment.workspace` | `false` | 保留 |
| 系统自动生成 `/tmp/myagent-workspace/{taskId}/` | `true` | 删除 |

> **重要**: 清理判断基于 `workspaceAutoGenerated` 标记，**不依赖路径前缀**。即使用户指定了 `/tmp/myagent-workspace/` 下的路径，也不会被误删。

### 在 Workflow YAML 中指定 Workspace

```yaml
# 通过 API 提交时指定
# POST /agent/execute
# { "workflow": "my-workflow", "environment": { "workspace": "/path/to/project" } }
```
