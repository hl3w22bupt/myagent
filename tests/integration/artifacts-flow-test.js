...
// 使用编译后的代码
import { WorkflowEngine } from '../.motia/compiled/src/core/workflow/engine.js';
import { AgentArtifacts } from '../.motia/compiled/src/core/agent/artifacts.js';
...
class MockAgent {
  ...
  updateLLMTraceConfig(taskId) {}
  setHookManager(hookManager) {}
}

// 创建 Mock AgentManager
const mockAgentManager = {
  acquire: async (agentId, options) => {
...
workflowEngine
  ...
  .then((result) => {
    ...
    const implementStep = result.steps.find((s) => s.stepId === 'implement');
    if (implementStep) {
