    ...
    def _convert_to_output_builder(self, llm_response: str) -> Dict[str, Any]:
        ...
        if not OUTPUT_BUILDER_AVAILABLE:
            ...
            if self._is_markdown(llm_response):
                ...
                # 仅当代码块占绝大部分时才判为 code，否则视为文本
                code_block = self._extract_code_block(llm_response)
                code_len = len(code_block['code'])
                total_len = len(llm_response.strip())
                surrounding_text_len = total_len - code_len - 10
                result_type = "code" if surrounding_text_len < total_len * 0.2 else "text"
            else:
                result_type = "text"
        ...
        # 1. 尝试提取代码块（仅当回复以代码块开头且不含其他分析内容时）
        # 如果回复除了代码块外还有大量文本（分析、说明等），说明这是一个包含代码片段的文本报告，
        # 而不是纯代码输出，应该作为 text/markdown 整体返回
        code_block = self._extract_code_block(llm_response)
        if code_block:
            # 检查代码块是否占回复的绝大部分（>80%），如果是才作为纯代码输出
            code_len = len(code_block['code'])
            total_len = len(llm_response.strip())
            # 去除代码块标记后的纯文本比例
            surrounding_text_len = total_len - code_len - 10  # ~10 chars for ``` marks
            is_mostly_code = surrounding_text_len < total_len * 0.2
