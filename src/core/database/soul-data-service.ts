 ...
 * Soul Data Service
 *
 * Data access layer for Soul agent state and context
 * Provides methods to save/load soul_states and soul_contexts
 */
...
import { UserProfile, RelationshipState } from '../context/soul-context-manager';
...
/**
 * Soul Context Data Service
 *
 * Manages soul_contexts table (business data, heavier)
 */
export class SoulContextDataService {
  /**
   * Save soul context to database
   *
   * @param sessionId - Session ID
   * @param userId - User ID
   * @param userProfile - User profile
   * @param relationshipState - Relationship state
   * @param conversationRounds - Conversation rounds
   */
  async saveSoulContext(
    sessionId: string,
    userId: string,
    ...
    try {
      await client.query(`
        INSERT INTO soul_contexts (session_id, user_id, conversation_rounds, user_profile, relationship_state, updated_at)
      ...
      `, [
        sessionId,
        userId,
        JSON.stringify(conversationRounds),
        JSON.stringify(userProfile),
        JSON.stringify(relationshipState)
      ]);

      console.log(`[SoulContextDataService] Saved soul context: ${sessionId}`);
    } catch (error: any) {
      console.error(`[SoulContextDataService] Failed to save soul context: ${error.message}`);
      throw error;
    } finally {
      client.release();
    }
   ...
   *
   * @param sessionId - Session ID
   * @returns Soul context or null if not found
   */
  async getSoulContext(sessionId: string): Promise<{
    userId: string;
    userProfile: UserProfile;
    relationshipState: RelationshipState;
    conversationRounds: any[];
    ...
    try {
      const result = await client.query(`
        SELECT session_id, user_id, conversation_rounds, user_profile, relationship_state
        FROM soul_contexts
        WHERE session_id = $1
      `, [sessionId]);
      ...
      return {
        ...
        conversationRounds: row.conversation_rounds || []
      };
    } catch (error: any) {
      console.error(`[SoulContextDataService] Failed to get soul context: ${error.message}`);
      throw error;
    } finally {
      client.release();
    }
   ...
   *
   * @param sessionId - Session ID
   * @param userProfile - User profile updates
   */
  async updateUserProfile(sessionId: string, userProfile: Partial<UserProfile>): Promise<void> {
    const store = getPostgresStore();
    await store.initialize();
    ...
    try {
      ...
      const result = await client.query(`
        SELECT user_id, conversation_rounds, user_profile, relationship_state
        FROM soul_contexts
        WHERE session_id = $1
      `, [sessionId]);

      if (result.rows.length === 0) {
        throw new Error(`Soul context not found: ${sessionId}`);
      }

      const row = result.rows[0];

      // Merge and save
      const mergedProfile = { ...row.user_profile, ...userProfile };

      await client.query(`
        UPDATE soul_contexts
        SET user_profile = $1, updated_at = CURRENT_TIMESTAMP
        WHERE session_id = $2
      `, [JSON.stringify(mergedProfile), sessionId]);

      console.log(`[SoulContextDataService] Updated user profile: ${sessionId}`);
    } catch (error: any) {
      console.error(`[SoulContextDataService] Failed to update user profile: ${error.message}`);
      throw error;
    } finally {
      client.release();
    }
   ...
   *
   * @param sessionId - Session ID
   * @param relationshipState - Relationship state updates
   */
  async updateRelationshipState(sessionId: string, relationshipState: Partial<RelationshipState>): Promise<void> {
    const store = getPostgresStore();
    await store.initialize();
    ...
    try {
      ...
      // Merge and save
      const mergedRelationship = { ...row.relationship_state, ...relationshipState };

      await client.query(`
        UPDATE soul_contexts
        SET relationship_state = $1, updated_at = CURRENT_TIMESTAMP
        WHERE session_id = $2
      `, [JSON.stringify(mergedRelationship), sessionId]);

      console.log(`[SoulContextDataService] Updated relationship state: ${sessionId}`);
    } catch (error: any) {
      console.error(`[SoulContextDataService] Failed to update relationship state: ${error.message}`);
      throw error;
    } finally {
      client.release();
    }
  ...
  async addConversationMessage(sessionId: string, role: 'user' | 'assistant', content: string): Promise<void> {
    ...
    try {
      ...
      if (result.rows.length === 0) {
        ...
        console.warn(`[SoulContextDataService] Creating new context for: ${sessionId}`);
        // Don't throw, just return without adding
        return;
      }

      const row = result.rows[0];

      // Add new conversation round
      const newRound = {
        role,
        content,
        timestamp: Date.now()
      ...
      const updatedRounds = [...row.conversation_rounds, newRound];

      // Keep only last 100 rounds
      const trimmedRounds = updatedRounds.slice(-100);

      await client.query(`
        UPDATE soul_contexts
        SET conversation_rounds = $1, updated_at = CURRENT_TIMESTAMP
        WHERE session_id = $2
      `, [JSON.stringify(trimmedRounds), sessionId]);

      console.log(`[SoulContextDataService] Added conversation message: ${sessionId}`);
    } catch (error: any) {
      console.error(`[SoulContextDataService] Failed to add conversation message: ${error.message}`);
      throw error;
    } finally {
      client.release();
    }
  ...
  async getRecentConversations(sessionId: string, limit: number = 10): Promise<Array<{
    ...
    try {
      ...
      const rounds = result.rows[0].conversation_rounds || [];

      // Return last N rounds
      return rounds.slice(-limit);
    } catch (error: any) {
      console.error(`[SoulContextDataService] Failed to get recent conversations: ${error.message}`);
      throw error;
    } finally {
      client.release();
    }
...
export const soulContextDataService = new SoulContextDataService();
