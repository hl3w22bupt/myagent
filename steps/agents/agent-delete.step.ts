...
/**
 * Agent Delete API Step configuration.
 */
export const config: ApiRouteConfig = {
  type: 'api',
  name: 'agent-delete-api',
...
export const handler = async (request: any, { logger, state }: any) => {
  ...
  logger.info('Agent Delete API: Received delete request', { taskId: id });
  ...
  try {
    ...
    // Update state
    await state.set(groupId, key, newHistory);

    logger.info('Agent Delete API: Task deleted successfully', {
      taskId: id,
    ...
    logger.error('Agent Delete API: Error deleting task', {
      error: error.message,
      stack: error.stack,
    });
