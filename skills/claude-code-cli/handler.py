        ...
        if exit_code == 0:
            ...
            try:
                ...
                if OUTPUT_BUILDER_AVAILABLE:
                    ...
                    # If workspace has files, return file type with output_files
                    # This tells task-result-handler to create artifacts for each file
                    if output_files:
                        # Build content with output_files for task-result-handler
                        content_with_files = {
                            "output_files": output_files,
                            "message": f"Created {len(output_files)} file(s) in workspace"
                        }

                        # If there's output_data, include it too
                        if isinstance(output_data, dict):
                            content_with_files.update(output_data)
                        elif isinstance(output_data, str):
                            content_with_files["output"] = output_data

                        builder.set_result_type('file')
                        builder.set_title(f"Generated {len(output_files)} file(s)")
                        # Manually set content to include output_files
                        result = builder.build()
                        result['content'] = content_with_files
                        write_structured_output(result, session_id)
                        return result
                    ...
                    # If workspace has files, include output_files in content
                    content = output_data
                    result_type = "code"
                    ...
                    if output_files:
                        ...
                        # Include original output_data if it's a dict
                        if isinstance(output_data, dict):
                ...
                if OUTPUT_BUILDER_AVAILABLE:
                    ...
                    # If workspace has files, include them in content
                    if output_files:
                        content_with_files = {
                            "output_files": output_files,
                            "message": f"Created {len(output_files)} file(s) in workspace",
