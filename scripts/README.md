# Pre-commit CI Check Script

在提交代码前运行所有 GitHub Actions CI 检查，确保代码质量。

## 使用方法

### 方式 1: 使用 npm 脚本 (推荐)

```bash
npm run pre-commit
```

### 方式 2: 直接运行脚本

```bash
bash scripts/pre-commit-check.sh
```

### 方式 3: 跳过测试 (仅检查类型和编译)

```bash
SKIP_TESTS=1 npm run pre-commit
```

## 检查内容

脚本按顺序执行以下检查：

1. ✅ **安装依赖** - `npm ci`
2. ✅ **生成 Motia 类型** - `npm run generate-types`
3. ✅ **TypeScript 类型检查** - `npx tsc --noEmit`
4. ✅ **ESLint 检查** - `npm run lint`
5. ✅ **TypeScript 编译** - `npm run build:ts`
6. ✅ **Jest 测试** - `npm run test -- --passWithNoTests` (可选)

## 输出示例

```
╔══════════════════════════════════════════════════════════════════════════════╗
║                                                                              ║
║  🚀 Pre-commit CI Check                                                      ║
║  Running all GitHub Actions checks locally                                   ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  1. Installing Dependencies (npm ci)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✓ Dependencies installed

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  2. Generating Motia Types
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✓ Motia types generated

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  3. TypeScript Type Check (tsc --noEmit)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✓ TypeScript type check passed

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  4. ESLint Check
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✓ ESLint passed (0 errors, 93 warnings)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  5. TypeScript Compilation (build:ts)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✓ TypeScript compilation successful

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  6. Jest Tests (Optional)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✓ Tests completed (91/160 passed)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Summary
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✓ All CI checks passed!

  Passed: 6
  Failed: 0

  Time taken: 45s

🎉 Safe to commit!
```

## 退出代码

- **0**: 所有检查通过 ✅
- **1**: 有检查失败 ❌

## Git Hook 集成 (可选)

如果你想在每次 commit 前自动运行检查，可以安装 husky:

```bash
npm install -D husky
npx husky install
npx husky add .husky/pre-commit "npm run pre-commit"
```

## 注意事项

1. **测试是可选的**: 默认会运行测试，但即使测试失败也不会阻止提交 (因为某些集成测试需要服务器运行)
2. **跳过测试**: 如果只想快速检查类型和编译，使用 `SKIP_TESTS=1`
3. **警告不阻止提交**: ESLint 的警告 (warnings) 不会导致检查失败，只有错误 (errors) 会

## 故障排查

### 问题: TypeScript 编译失败
```bash
# 清理并重新安装
npm run clean
npm install
npm run pre-commit
```

### 问题: 类型检查错误
```bash
# 查看详细错误
npx tsc --noEmit
```

### 问题: 测试超时
```bash
# 跳过测试
SKIP_TESTS=1 npm run pre-commit
```
